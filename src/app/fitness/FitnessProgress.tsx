...
import { CenteredModal, LogWeightModal, modalInputStyle, modalLabel } from './fitnessModals';
import {
  addDays, completionsThisWeek, startOfDay, startOfWeek, weeklyVolumeSeries, weekStreaks,
} from './progressStats';
...
export default function FitnessProgress() {
  ...
  const completedThisWeek = useMemo(() => completionsThisWeek(completions, today), [completions, today]);

  const { currentStreak, bestStreak } = useMemo(() => weekStreaks(completions, today), [completions, today]);
  ...
  const strength = useMemo(() => {
    ...
    const weeklyVolume = weeklyVolumeSeries(setLogs, today, VOL_WEEKS);
