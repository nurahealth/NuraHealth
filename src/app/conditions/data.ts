// ─────────────────────────────────────────────────────────────────────────────
// Conditions — the typed content layer for /conditions.
//
// Everything a condition screen renders lives here as data: the plain-language
// intro, the marker rules that decide whether a condition rises to the top of
// the index, the numbered protocol, and the per-step detail card.
//
...
//     the NŪRA book + marketing landing page that each condition will pair
//     with. Null until that asset exists.
//
// Rich text: `**bold**` inside `intro`, `matchedIntro`, `how`, `extra.body` and
// `checkFirst` is rendered as emphasis. Nothing else is parsed.
// ─────────────────────────────────────────────────────────────────────────────

/** Icon key → a lucide icon, resolved in icons.tsx. */
export type ConditionIcon =
  | "zap" | "moon" | "leaf" | "waves"
  | "activity" | "shield" | "heart" | "bug" | "droplets";
...
export interface Condition {
  slug: string;
  name: string;
  /** Trailing words of the title, set in italic sage/ink. */
  nameEmphasis?: string;
  icon: ConditionIcon;
  /** One line under the name in search results and the index row. */
  blurb: string;
  ...
  /** "What your body is doing", always shown. */
  intro: string;
...
export const CONDITIONS: Condition[] = [
  // ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    intro:
      "Persistent tiredness is rarely about willpower. Most of the time it is one of a short list of things: blood sugar that swings instead of holding steady, a nutrient you are genuinely low on, sleep that looks long enough but isn't restorative, or a thyroid that has quietly drifted. **High triglycerides with low HDL is one of the clearest patterns** — it usually means your body is handling carbohydrate and fat less efficiently than it could, and the afternoon crash is what that feels like from the inside. The good news is that this cluster moves faster than almost anything else on a lab panel.",
    matchedIntro:
    ...
    steps: [
      ...
      },
      {
        ...
        how: "**Ten minutes outside**, no sunglasses, ideally before 9 a.m. An overcast morning still delivers many times more light than a bright room. Walking while you do it counts twice — see the movement step.",
        evidence: [
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    icon: "moon",
    blurb: "The one input that makes every other pillar easier.",
    matchRules: [],
    pillarRule: { key: "sleep", label: "Sleep", below: 80 },
    intro:
      "Sleep is not one thing going wrong — it is usually timing, pressure, or arousal. **Timing** is your body clock sitting later than your alarm. **Pressure** is how much sleep drive you've built up, which caffeine and long naps quietly drain. **Arousal** is the wired-and-tired state where the lights are off and your nervous system hasn't got the message. Most of what follows is aimed at the first two, because they are mechanical and they respond fast. The third one has a real treatment, and it isn't a supplement.",
    matchedIntro:
    ...
    steps: [
      ...
      },
      {
        ...
        how: "Count back **8 to 10 hours from your bedtime** and make that your last cup. If you wake at 6 and sleep at 10, that's a noon cutoff. Watch the hidden sources: green tea, pre-workout, dark chocolate, and most energy drinks carry far more than a coffee.",
        extra: {
          label: "The tell",
      ...
      },
      {
        ...
        supervised: true,
        how: "If you've had trouble falling or staying asleep **three nights a week for three months or more**, ask your doctor for a referral to CBT-I — cognitive behavioural therapy for insomnia. It runs four to eight sessions, in person or through a validated app, and the gains hold after it ends in a way medication doesn't.",
        evidence: [
          "American Academy of Sleep Medicine — clinical practice guideline on behavioural and psychological treatment of chronic insomnia.",
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    matchRules: [],
    intro:
      "Most digestive complaints that aren't a diagnosed disease come down to three things: **not enough fibre, not enough variety, and too little time**. The bacteria in your colon live on the parts of plants you can't digest; feed them a narrow diet and the community narrows with it. The fix is genuinely dull — more plants, more different plants, introduced slowly enough that the first two weeks aren't miserable. What follows is aimed at everyday bloating, irregularity and discomfort. It is not aimed at inflammatory bowel disease, coeliac disease or anything with blood in it.",
    signals: [
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    intro:
      "Chronic stress isn't only a feeling — it's a physiology. Sustained load keeps cortisol and adrenaline elevated, which shortens sleep, raises blood pressure and blood sugar, and makes the next stressor land harder. That's the loop worth breaking, and the honest ranking of what breaks it is: **movement, sleep, and connection first; breathing practices second; herbs a distant third.** Anything that sells you the third without the first two is selling you something.",
    matchedIntro:
    ...
    steps: [
      ...
      },
      {
        ...
        how: "Typical trials use a **standardised root extract, 300–600 mg a day, for 8 to 12 weeks**. Take it with food. If nothing has shifted by twelve weeks, stop — this isn't a compound to take indefinitely on hope.",
        evidence: [
      ...
      },
      {
        ...
        supervised: true,
        how: "If low mood, or loss of interest in things you normally enjoy, has been present **most of the day, most days, for two weeks or more**, that's the threshold clinicians use. Say it plainly to your doctor. Effective treatment exists and it works better the earlier it starts.",
        evidence: [
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    intro:
      "The most counter-intuitive fact about aching joints is that **rest is usually the wrong prescription**. For osteoarthritis, the strongest recommendation in every major guideline is exercise — loading the joint and strengthening what surrounds it. Cartilage has no blood supply of its own; movement is how it gets fed. Where supplements sit in all of this is honestly quite low, and the marketing around them is far ahead of the evidence.",
    signals: [
      { markerId: "hs-crp", label: "hs-CRP", unit: "mg/L", flagAbove: 3 },
      { markerId: "uric-acid", label: "Uric acid", unit: "mg/dL", flagAbove: 7 },
      { markerId: "vit-d", label: "Vitamin D", unit: "ng/mL", flagBelow: 30 },
    ...
    steps: [
      ...
      },
      {
        ...
        grade: "A",
        gradeNote: "ACR / Arthritis Foundation guideline",
        how: "**5 to 10% of body weight** is the threshold where the guidelines report meaningful symptom change. Pair it with the strength work — losing weight without loading the joint gives away muscle you need. If this doesn't apply to you, skip it; it isn't a moral step.",
        evidence: [
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    intro:
      "\"Immune support\" is the most oversold phrase in the supplement aisle. You cannot boost an immune system — an over-active one is an autoimmune disease. What you can do is remove the things that measurably suppress it and correct genuine deficiencies. **Sleep, vaccination and handwashing are the three highest-evidence interventions on this page**, and none of them are for sale. The supplements below are real but small, and they matter most in people who are actually short of something.",
    matchedIntro:
    ...
    steps: [
      {
        ...
        how: "**Seven hours or more, consistently.** Around a vaccination it matters even more — short sleep the nights either side is associated with a weaker antibody response. Take the sleep protocol in this section rather than a supplement if you have to pick one.",
        evidence: [
      ...
      },
      {
        ...
        how: "**Start within 24 hours of the first symptom** and use lozenges that dissolve slowly in the mouth, not swallowed capsules. Stop when the cold does. This is a short course, not a daily habit.",
        extra: {
      ...
      },
      {
        ...
        how: "**Twenty seconds with soap and water**, especially before eating and after public transport, shared keyboards and bathrooms. Alcohol gel at 60%+ when there's no sink. Keep your hands off your face — that's the actual route in.",
        evidence: [
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    intro:
      "Cardiovascular risk is built from a small number of things you can actually see: **how many atherogenic particles are circulating** (ApoB, or non-HDL cholesterol as a stand-in), your blood pressure, whether you smoke, and how your body handles sugar. Triglycerides and HDL move quickly with diet and movement — often within weeks. LDL and ApoB move more slowly and are more genetic. Knowing which of your numbers is which tells you where effort is worth spending.",
    matchedIntro:
    ...
    steps: [
      {
        ...
        how: "**150 minutes of moderate activity a week** — brisk enough that talking is possible and singing isn't — or 75 minutes of vigorous. Add **two resistance sessions**. Spread it across the week rather than banking it all on Sunday.",
        evidence: [
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    intro:
      "Alpha-gal syndrome is an allergy to a sugar called **galactose-α-1,3-galactose** — found in most mammals, but not in people. In the US it usually begins after a bite from a lone star tick. Your immune system starts treating that sugar as a threat, so beef, pork, lamb and venison can set off a reaction. The timing is what throws people: symptoms often arrive **two to six hours after eating**, so the meal rarely gets blamed.",
    matchedIntro:
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    pillarRule: { key: "metabolic", label: "Metabolic", below: 75 },
    intro:
      "This protocol **supports** the care you're getting from your doctor. It does not treat, reverse or cure diabetes, and nothing here is a reason to change or stop a medication — that decision is your prescriber's alone, always. What these steps do is well documented: the daily habits around meals, movement and sleep have a real, measurable effect on blood glucose, and they work alongside whatever your care team has prescribed. **The medication and the habits are not competing with each other.**",
    matchedIntro:
    ...
    steps: [
      {
        ...
        grade: "B+",
        gradeNote: "Evidence summary — systematic reviews",
        how: "**Ten to fifteen minutes, starting within about half an hour of finishing.** Easy pace — this isn't exercise, it's a glucose-disposal errand. After the largest or most carbohydrate-heavy meal is where it earns the most. Do it after every meal if you can; after dinner if you can only manage one.",
        evidence: [
      ...
      },
      {
        ...
        how: "**Two to three sessions a week**, covering the major muscle groups, alongside the ADA's 150 minutes of aerobic activity. Bands and bodyweight count. Don't go more than two days without some activity — the insulin-sensitivity benefit of a session fades within about 48 hours.",
        evidence: [
      ...
      },
      {
        ...
        how: "**Seven hours or more, at a consistent time.** The ADA now includes sleep assessment in routine diabetes care for exactly this reason. Take the sleep protocol in this section and run the fixed wake time and the caffeine curfew first.",
        evidence: [
