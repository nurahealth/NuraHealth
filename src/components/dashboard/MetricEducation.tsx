...
export default function MetricEducation({
  ...
  return (
    <section style={{ background: SURFACE, border: `0.5px solid ${HAIR}`, borderRadius: 18, padding: "4px 18px", fontFamily: SANS }}>
      <div style={{ padding: "16px 0 4px", fontSize: 14, fontWeight: 600, color: CREAM }}>{title}</div>

      {items.map((it) => (
        <div key={it.label} style={{ padding: "16px 0", borderTop: `0.5px solid ${HAIR}` }}>
          <div style={{ fontSize: 10.5, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.12em", color: accent, marginBottom: 7 }}>{it.label}</div>
