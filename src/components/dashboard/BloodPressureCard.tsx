  ...
  return (
    <div style={{ marginTop: 14 }}>
      ...
      <div style={{ position: "relative", height: 7 }}>
        <div className="nura-meter-track" style={{ position: "absolute", inset: 0, borderRadius: 999, background: gradient }} />
        <div className="nura-meter-tick" style={{ position: "absolute", top: -1, left: `${tickPct}%`, transform: "translateX(-50%)", width: 1.5, height: 9, borderRadius: 1, background: "var(--nura-tick-on-accent)" }} />
        <div className="nura-meter-dot" style={{ position: "absolute", top: "50%", left: `${pct.toFixed(1)}%`, transform: "translate(-50%,-50%)", width: 4, height: 15, borderRadius: 2.5, background: "var(--nura-marker)", boxShadow: `0 0 0 2.5px ${CARD}` }} />
      </div>
    </div>
  );
}
